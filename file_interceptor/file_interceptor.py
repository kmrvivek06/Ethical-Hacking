...
def get_argument():
    ...
    parser.add_argument("-t", "--target", dest="target", help="Target file path")
...
def process_packet(packet):
    ...
    if scapy_packet.haslayer(scapy.Raw) and scapy_packet.haslayer(scapy.TCP):
        if scapy_packet[scapy.TCP].dport == 80:
            if file_type in scapy_packet[scapy.Raw].load and target_file not in scapy_packet[scapy.Raw].load:
